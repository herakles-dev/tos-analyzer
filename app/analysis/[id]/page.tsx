...
function getAgeBanner(createdAt: string | undefined): { label: string; level: 'stale' | 'very-stale' } | null {
  if (!createdAt) return null;
  const ageMonths = (Date.now() - new Date(createdAt).getTime()) / MS_PER_MONTH;
  if (ageMonths >= VERY_STALE_MONTHS) return { label: `${Math.floor(ageMonths)} months old`, level: 'very-stale' };
  if (ageMonths >= STALE_MONTHS) return { label: `${Math.floor(ageMonths)} months old`, level: 'stale' };
  return null;
}
...
export default function AnalysisPage() {
  ...
  const riskScore = calculateRiskScore(summary.red_count, summary.yellow_count, summary.green_count);
  ...
  return (
    <main className="analysis-page">
      ...
      <div className="analysis-ai-disclaimer">
        <span className="analysis-ai-disclaimer__badge">AI-generated</span>
        <span className="analysis-ai-disclaimer__text">
          This analysis was generated by AI from the document you provided. It
          may contain errors, omissions, or misinterpretations — always read
          the original Terms of Service for binding legal terms.
        </span>
      </div>
      ...
      <div className="analysis-content">
        {(() => {
          const validation = analysis.analysis.document_validation;
          const totalClauses = summary.total_clauses ?? 0;
          const isIncomplete = validation?.is_complete_document === false;
          const tooFewClauses = totalClauses < 3;
          if (!isIncomplete && !tooFewClauses) return null;

          const issues = validation?.content_issues || [];
          const reason = validation?.rejection_reason;
          let body: string;
          if (isIncomplete) {
            body = reason
              || "This appears to be an introductory or stub page (such as a navigation hub or summary) rather than the full Terms of Service. The actual binding legal document is likely linked from the page you analyzed. For a complete analysis, please paste the full TOS text.";
          } else {
            body = `Only ${totalClauses} substantive clause${totalClauses === 1 ? '' : 's'} were found in this document. The text may be truncated, an excerpt, or otherwise incomplete. For best results, paste the complete legal document.`;
          }

          return (
            <div className="analysis-incomplete-banner">
              <AlertTriangle className="w-5 h-5 flex-shrink-0" />
              <div>
                <strong>Incomplete document detected.</strong>{' '}
                {body}
                {issues.length > 0 && (
                  <span className="analysis-incomplete-banner__tags">
                    {issues.map((tag) => (
                      <span key={tag} className="analysis-incomplete-banner__tag">{tag.replace(/_/g, ' ')}</span>
                    ))}
                  </span>
                )}
                <div className="analysis-incomplete-banner__hint">
                  This analysis is not eligible for the public library.
                </div>
              </div>
            </div>
          );
        ...
        {(() => {
          const ageBanner = getAgeBanner(analysis.created_at);
          if (!ageBanner) return null;
          return (
            <div className={`analysis-age-banner analysis-age-banner--${ageBanner.level}`}>
              <Clock className="w-4 h-4 flex-shrink-0" />
              <div>
                <strong>This analysis is {ageBanner.label}.</strong>{' '}
                {ageBanner.level === 'very-stale'
                  ? 'The Terms of Service have likely changed substantially since then. Treat this as a historical reference.'
                  : 'The Terms of Service may have changed since this was analyzed.'}
              </div>
            </div>
          );
        })()}
